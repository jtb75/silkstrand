#!/usr/bin/env python3
"""CIS MongoDB 8 Benchmark — SilkStrand compliance bundle entry point."""
...
def main() -> None:
    ...
    target_identifier = f"{host}:{port}"
    _log(f"connecting to {target_identifier} as {username or '(no auth)'}")
    ...
    try:
        ...
        # Force a connection-time error surfacing
    ...
    try:
        controls = evaluator.evaluate_all(client, CONTROLS_DIR)
    finally:
        try: client.close()
