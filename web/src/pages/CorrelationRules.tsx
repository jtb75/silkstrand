...
// comes later). The API validates the shape on POST so a bad body
// surfaces a 400.
...
function RuleForm({ submitting, error, onSubmit }: FormProps) {
  const [bodyText, setBodyText] = useState(EXAMPLE_BODY);
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setParseErr(null);
    onSubmit({ name, trigger, enabled: true, body: parsed });
  }
  ...
  return (
    <form className="form-card" onSubmit={handleSubmit}>
      <div className="form-group">
        ...
        <input id="name" name="name" required placeholder="cis-postgres-16-suggest" />
      </div>
      <div className="form-group">
        <label htmlFor="trigger">Trigger</label>
        <select id="trigger" name="trigger" defaultValue="asset_discovered">
