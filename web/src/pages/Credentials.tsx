          ...
          <tbody>
            {sources.map((s) => (
              <SourceRow
                ...
                onDelete={() => {
                  if (!window.confirm(`Delete ${s.type} credential source?`)) return;
...
function SourceRow({
  ...
  return (
    <>
      ...
      {showMapModal && (
        <tr>
          <td colSpan={supportsMappings ? 4 : 3}>
            <MapToCollectionPanel
              sourceId={source.id}
              existingMappingIds={new Set(mapped.map((m) => m.collection_id))}
...
function renderConfigSummary(s: CredentialSource): string {
  ...
  if (s.type === 'static') {
    ...
    return `type=${t}, encrypted_data=(set)`;
  }
  if (s.type === 'webhook') {
    ...
    <div className="form-card" style={{ marginTop: 8 }}>
      ...
      <ul style={{ listStyle: 'none', padding: 0, margin: '8px 0' }}>
        {endpointCollections.map((c) => {
          const already = existingMappingIds.has(c.id);
          return (
...
function CredentialSourceForm({
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    switch (type) {
      case 'static': {
        setErr('Static credentials are created via a target\'s Credential button.');
        return;
      }
      case 'webhook': {
  ...
  return (
    <form className="form-card" onSubmit={handleSubmit}>
      ...
      {type === 'static' && (
        <p className="muted">Static DB / host credentials are created per target via the Targets page.</p>
      )}
      {type === 'webhook' && (
        <>
