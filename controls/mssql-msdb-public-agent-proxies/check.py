#!/usr/bin/env python3
"""Control: mssql-msdb-public-agent-proxies — Ensure the public role in msdb is not granted access to SQL Agent proxies"""
...
QUERY = """USE [msdb]; SELECT sp.name AS proxyname FROM dbo.sysproxylogin spl JOIN sys.database_principals dp ON dp.sid = spl.sid JOIN sysproxies sp ON sp.proxy_id = spl.proxy_id WHERE dp.principal_id = USER_ID('public');"""
