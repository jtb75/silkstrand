#!/usr/bin/env python3
"""Control: mssql-orphaned-users — Ensure Orphaned Users are Dropped From SQL Server Databases"""
...
QUERY = """SELECT dp.name AS orphaned_user FROM sys.database_principals dp LEFT JOIN sys.server_principals sp ON dp.sid = sp.sid WHERE dp.type IN ('S','U','G') AND dp.authentication_type_desc = 'INSTANCE' AND sp.sid IS NULL AND dp.name NOT IN ('dbo','guest','INFORMATION_SCHEMA','sys','MS_DataCollectorInternalUser');"""
