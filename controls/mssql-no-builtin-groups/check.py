#!/usr/bin/env python3
"""Control: mssql-no-builtin-groups — Ensure Windows BUILTIN groups are not SQL Logins"""
...
QUERY = """SELECT pr.[name], pe.[permission_name], pe.[state_desc] FROM sys.server_principals pr JOIN sys.server_permissions pe ON pr.principal_id = pe.grantee_principal_id WHERE pr.name LIKE 'BUILTIN%';"""
