#!/usr/bin/env python3
"""Control: mssql-max-error-log-files — Ensure Maximum number of error log files is set to 12 or greater"""
...
QUERY = r"""DECLARE @numErrorLogs int; EXEC master.sys.xp_instance_regread N'HKEY_LOCAL_MACHINE', N'Software\Microsoft\MSSQLServer\MSSQLServer', N'NumErrorLogs', @numErrorLogs OUTPUT; SELECT ISNULL(@numErrorLogs, -1) AS [NumberOfLogFiles];"""

ASSERTIONS = [
    ("value", "greater_than_or_equal", "12"),
]
...
def main():
    ...
    # Normalize first column to 'value'
    first_col = next(iter(rows[0]))
    for row in rows:
        row["value"] = row[first_col]
